...
class SwatplusLulcChangeProcessor(BaseProcessor):
    ...
    def execute(self, data, outputs=None):
        ...
        in_project = data.get('input_project_url')
        in_lulc_changes_csv = data.get('lulc_changes_csv_url')

        # Set defaults:
        if in_project is None:
            raise ProcessorExecuteError('Missing parameter: input_project_url')
        if in_lulc_changes_csv is None:
            raise ProcessorExecuteError('Missing parameter: lulc_changes_csv_url')

        if not in_project.startswith('http'):
            raise ProcessorExecuteError('Should be a URL: parameter input_project_url')
        if not in_lulc_changes_csv.startswith('http'):
            raise ProcessorExecuteError('Should be a URL: parameter lulc_changes_csv_url')
        ...
        result_file_name = 'modified.zip'
        downloadlink = f'{output_url}/{result_file_name}'
...
def run_docker_container(
    ...
    LOGGER.debug(f'Will use this image: {image_name}')
